# SignWell.Sdk.Models.DocumentFromTemplateRequest
...
**TemplateId** | **Guid** | Use when you have to create a document from a single template. Either :template_id or :template_ids must be present in the request, not both. | [optional] 
**TemplateIds** | **List&lt;Guid&gt;** | Use when you have to create a document from multiple templates. Either :template_id or :template_ids must be present in the request, not both. | [optional] 
...
**TemplateFields** | [**List&lt;TemplateFieldValuesInner&gt;**](TemplateFieldValuesInner.md) | Fields of your template(s) that you can prepopulate with values. Signature and Initials fields cannot be signed through the API. | [optional] 
